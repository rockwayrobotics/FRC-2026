...
public class ModuleIOSparkAbsolute implements ModuleIO {
  ...
  private final SparkAnalogSensor turnAnalog; // TURNIP
  ...
  public ModuleIOSparkAbsolute(SwerveModuleConfig module) {
    ...
    turnAnalog = turnSpark.getAnalog();
    ...
    turnConfig
        ...
        .uvwAverageDepth(2);
    double conversionFactor = (2 * Math.PI / 3.3) * (module.encoderInverted() ? -1 : 1);
    turnConfig.analogSensor.positionConversionFactor(
        conversionFactor); // FIXME: Possibly 2 * Pi? Or 2 * Pi / 3.3?
    turnConfig
        .closedLoop
        .feedbackSensor(FeedbackSensor.kPrimaryEncoder) // TURNIP: FeedbackSensor.kAnalogSensor)
    ...
    turnPositionQueue =
        SparkOdometryThread.getInstance()
            .registerSignal(turnSpark, turnEncoder::getPosition); // TURNIPturnAnalog::getPosition);
  }

  @Override
  public void updateInputs(ModuleIOInputs inputs) {
    // Reset the relative encoder frequently to match the absolute encoder.
    resetCounter++;
    if (resetCounter > 29) {
      turnSpark.getEncoder().setPosition(turnAnalog.getPosition());
      resetCounter = 0;
    }
    // Update drive inputs
